...
This module contains tests for the physics simulation of the brick.
"""
...
def generate_a_random_physics_engine():
    ...
    Creates a `World` instance with random initial conditions for the brick
...
def test_brick_property():
    """
    Test that the `brick` matches the initial position.

    Runs `NUM_EXP` experiments, each time generating a random `World` instance
    and checking if the `brick` property correctly reflects the initial brick
    position.
    """
    for i in range(NUM_EXP):
        physics, brick, _, _, _ = generate_a_random_physics_engine()
...
def test_brick_setter():
    ...
    Runs `NUM_EXP` experiments, each time setting a new brick position in a
    random `World` instance.
    Verifies that:
    ...
    """
    for i in range(NUM_EXP):
        physics, _, _, _, _ = generate_a_random_physics_engine()
...
def test_brick_drop():
    """
    Test the `drop` function with and without a z-axis limit.

    Runs `NUM_EXP` experiments, checking:
        1. Without a `z_limit`, ensuring the brick moves along the x, y, and z
           axes.
        2. With a `z_limit`, ensuring the brick movement stops at the
           specified limit on the z-axis.

    Verifies that the brick's position updates correctly according to the
    calculated velocities and timestep.
    """
    for i in range(NUM_EXP):
